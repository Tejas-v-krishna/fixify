...
const LightPillar = ({
    ...
    style = {}, // unused but kept for prop compatibility
    glowAmount = 0.005,
    pillarWidth = 3.0,
    pillarHeight = 0.4,
    noiseIntensity = 0.5,
    ...
    if (!webGLSupported) {
        return (
            <div className={`light-pillar-fallback ${className}`} style={{ mixBlendMode: 'normal' as any, ...style }}>
                WebGL not supported
            </div>
        );
    }

    return <div ref={containerRef} className={`light-pillar-container ${className}`} style={{ mixBlendMode: mixBlendMode as any, ...style }} />;
};
